...
import { getBinance } from '../libOld/binanceOld.js';
...
test("Returns price if Binance request succeeds", async () => {
    const getBinance = require('../libOld/binanceOld.js').getBinance // your function name could be different
...
test("Returns Message for Failed Binance Response (Same as Mock Value)", async () => {
    const getBinance = require('../libOld/binanceOld.js').getBinance
    jest.mock('node-binance-api', () => {
    return class Binance {
      prices() {
