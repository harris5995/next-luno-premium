...
export async function lunoToUSD(currency) {
    try {
      const lunoMYR = await luno(currency);
...
export async function priceDifference(currency) {
    try {
        const luno = await lunoToUSD(currency);
        const binance = await getBinance(currency);
...
export async function lunoPremium(currency) {
    try {
      const Luno = await lunoToUSD(currency);
      const price = await priceDifference(currency);
